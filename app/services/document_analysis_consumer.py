    ...
    async def _run_analysis(
        ...
        try:
            ...
            for i, batch in enumerate(batches):
                ...
                await db_service.save_extraction_result(project_id, chars, evts, stgs)
                ...
                # To avoid OOM, maybe only keep IDs?
                # But callback expects full objects.
                # If "Les Miserables" chars list is huge?
                # We trust "Global Merge" to clean up later.
                # For the callback, we just send what we found in this run?
                # Spring backend might overwrite?
                # Let's accumulate.
                for c in chars:
                    # FullCharacter uses profile.name, but some formats use root name
                    c_name = c.get("name") or c.get("profile", {}).get("name")
                ...
                if rel_graph and isinstance(rel_graph, dict):
                    rels = rel_graph.get("relationships", [])
                    if rels:
