...
class CallbackClient:
    ...
    async def send_analysis_callback(
        ...
        """
        # PRIORITIZE: Use the callback URL provided in the message if available
        if callback_url:
            url = callback_url
            # Docker environment compatibility adjustment if needed
            if "localhost" in url or "127.0.0.1" in url:
                # Check if we are running in docker container (simple check via settings or hostname,
                # but here we just apply the same fix as in _send_request_with_retry if needed,
                # although _send_request_with_retry already handles it.
                # So we just pass it through.)
                pass
            logger.info("Using provided callback_url from message", url=url)
        else:
            # Fallback to default constructed URL
            url = f"{settings.spring_callback_url}/api/internal/ai/analysis/callback"
            logger.info("Using default constructed callback URL", url=url)
        ...
        print(f"[CALLBACK] Sending callback to {url}, job_id={job_id}, status={status}", flush=True)
        ...
        try:
            ...
            if response.status_code in (200, 201, 202):
                ...
                print(f"[CALLBACK] SUCCESS! Response code: {response.status_code}", flush=True)
                return True
            else:
                logger.error(
                    "Callback failed",
                ...
                print(f"[CALLBACK] FAILED! Response code: {response.status_code}, body: {response.text[:500]}", flush=True)
                return False

        except Exception as e:
            logger.error("Callback failed after retries", job_id=job_id, error=str(e))
            print(f"[CALLBACK] EXCEPTION! Error: {str(e)}", flush=True)
            return False
